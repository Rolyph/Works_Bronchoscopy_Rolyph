...
def compute_metrics(pred, target, thresh=0.5):
    """
    Calcule plusieurs métriques pour évaluer la segmentation.

    Args:
        pred (torch.Tensor): Prédictions du modèle.
        target (torch.Tensor): Vérités terrain.
        thresh (float): Seuil pour binariser les prédictions.

    Returns:
        dict: Dictionnaire contenant Dice Score, IoU, Précision et Rappel.
    """
    metrics = {
        "Dice Loss": Threshold_DiceLoss(pred, target, thresh=thresh).item(),
        "IoU Loss": Threshold_IoULoss(pred, target, thresh=thresh).item(),
        "Precision": custom_precision_score(pred, target, thresh=thresh).item(),
        "Recall": custom_recall_score(pred, target, thresh=thresh).item(),
    }

    # Calculer le Dice Score en inversant la Dice Loss
    metrics["Dice Score"] = 1 - metrics["Dice Loss"]
    metrics["IoU"] = 1 - metrics["IoU Loss"]
