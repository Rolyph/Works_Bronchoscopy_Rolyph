...
def swin_v2(size, checkpoint_path, img_size=384, in_22k=False, **kwargs):
    # Vérifiez que la taille correspond au modèle attendu
    if size == "swinv2_base_window12to24_192to384_22kft1k":
        ...
        if checkpoint_path and os.path.exists(checkpoint_path):
            ...
            # Vérifiez si les poids sont encapsulés ou non
            ...
            try:
                model.load_state_dict(state_dict, strict=False)  # Strict=False pour éviter les erreurs mineures
                print(f"Loaded pretrained weights from {checkpoint_path}")
            except RuntimeError as e:
                print(f"Error loading state_dict: {e}")
                print("Proceeding with random initialization.")
        else:
            print("No valid checkpoint provided. Initializing the model with random weights.")
...
class FCBSwinV2_Transformer(nn.Module):
    def __init__(self, size=384, checkpoint_path=None):
        ...
        # Vérification du checkpoint_path
        if checkpoint_path and os.path.exists(checkpoint_path):
            print(f"Using checkpoint: {checkpoint_path}")
        else:
            checkpoint_path = None
            print("No valid checkpoint provided. Initializing random weights.")
