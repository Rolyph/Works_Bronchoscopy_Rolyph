...
parser = argparse.ArgumentParser(description="Data Loader for Lung Cancer Lesion Segmentation")
...
parser.add_argument('--image_size', type=int, required=True, help='Image resizing size')
...
def create_csv_from_folder(folder_path, csv_name):
    ...
    # Retrieve image and mask files
    all_images = glob.glob(os.path.join(images_folder, "*.png"))
    all_masks = glob.glob(os.path.join(masks_folder, "*.png"))
    ...
    # Sort and match images with masks
    image_names = sorted(os.path.basename(img) for img in all_images)
    mask_names = sorted(os.path.basename(msk) for msk in all_masks)
    
    # Create CSV with image_path and mask_path
    data = {
        'image_path': [os.path.join(images_folder, img) for img in image_names],
        'mask_path': [os.path.join(masks_folder, msk) for msk in mask_names]
    }
...
def load_data_to_model(image_size, csv_path):
    """
    ...
    X = np.zeros((len(all_images), image_size, image_size, 3), dtype=np.float32)
    Y = np.zeros((len(all_images), image_size, image_size), dtype=np.uint8)
    ...
    for n, (image_path, mask_path) in tqdm(enumerate(zip(all_images, all_masks)), 
        ...
        try:
            ...
            mask = np.array(Image.open(mask_path).resize((image_size, image_size)))
        ...
        # Normalize images
        X[n] = image / 255.0  # Values in range [0,1]
        Y[n] = mask  # Binary mask values [0,1]

        IDs.append(os.path.basename(image_path))

    Y = np.expand_dims(Y, axis=-1)  # Add channel dimension for PyTorch
    return IDs, X, Y


class LungLesionDataset(Dataset):
    """
    Custom dataset class for lung cancer lesion segmentation.
    """
